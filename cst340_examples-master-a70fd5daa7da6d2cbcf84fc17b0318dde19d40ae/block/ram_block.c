...
void rb_close(struct gendisk * disk, fmode_t mode)
{
...
static int rb_transfer(struct request *req)
{
    ...
    printk(KERN_DEBUG "rb: Dir:%d; Sec:%lu; Cnt:%d\n", dir, start_sector, sector_cnt);
...
static int __init rb_init(void)
{
    ...
    printk(KERN_INFO "rb: Calling RAM init function.");
    if ((ret = ramdevice_init()) < 0)
    {
        printk(KERN_ERR "rb: Unable to initialize RAM\n");
        return ret;
    }
    rb_dev.size = ret;
